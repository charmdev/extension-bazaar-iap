...
public class IabHelper {
    ...
    public void querySkuDetailsAsync(final List<String> moreSkus, final QuerySkuDetailsFinishedListener listener) {
        final Handler handler = new Handler();
        checkNotDisposed();
        checkSetupDone("querySkuDetails");
        iabConnection.flagStartAsync("query sku details");

         (new Thread(new Runnable() {
            public void run() {
                IabResult result = new IabResult(BILLING_RESPONSE_RESULT_OK, "Query sku details success.");
                Inventory inv = new Inventory();
                int r = BILLING_RESPONSE_RESULT_OK;
                try {
                    try {
                        r = querySkuDetails(ITEM_TYPE_INAPP, inv, moreSkus);
                    }
                    catch (RemoteException e) {
                        throw new IabException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while sku details.", e);
                    } 
                    catch (JSONException e) {
                        throw new IabException(IABHELPER_BAD_RESPONSE, "Error parsing JSON response while sku details.", e);
                    }
